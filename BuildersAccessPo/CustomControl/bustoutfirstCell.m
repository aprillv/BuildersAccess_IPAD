...
@implementation bustoutfirstCell{
    BOOL noup;
    BOOL dateup;
    BOOL projectup;
}
...
- (void)drawRect:(CGRect)rect{
    ...
    btn1 = [[UIButton alloc] initWithFrame: CGRectMake( tx, 0, dw*.45+2, 44)];
...
-(void)idclick{
    noup=!noup;
    if (delegate) {
        return [delegate doaClicked:@"refdate" :noup];
    }
}
-(void)pronameclick{
    dateup=!dateup;
...
-(void)pronameclick1{
    projectup=!projectup;
    if (delegate) {
        return [delegate doaClicked:@"Sales1" :projectup];
    }
}
