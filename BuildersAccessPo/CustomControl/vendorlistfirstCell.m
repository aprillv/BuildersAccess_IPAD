...
@implementation vendorlistfirstCell{
    BOOL idup;
    BOOL cnameup;
}
...
- (void)drawRect:(CGRect)rect{
    ...
    UIButton *btn1 = [[UIButton alloc] initWithFrame: CGRectMake( tx, 0, dw*.6+2, 44)];
    ...
    [btn1 setTitle:@"Contact" forState:UIControlStateNormal];
    //     [btn1.titleLabel setFont:[UIFont boldSystemFontOfSize:17.0]];
    [btn1 setBackgroundColor:[UIColor lightGrayColor]];
    //     btn1.autoresizingMask=UIViewAutoresizingFlexibleWidth;
    [btn1 setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
...
-(void)idclick{
    idup=!idup;
    if (delegate) {
        return [delegate doaClicked:@"Name" :idup];
    }
}
-(void)pronameclick{
    cnameup=!cnameup;
    if (delegate) {
        return [delegate doaClicked:@"Contact" :cnameup];
    }
}
